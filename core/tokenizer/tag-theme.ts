/**
 * Lezer-tag-aware theme resolution.
 *
 * Split from `token-theme.ts` so consumers that only use scope-string
 * resolution (Rust tokenizer bridge, tree-sitter engine) don't transitively
 * import `@lezer/highlight`. The Perry-compiled web target builds without
 * any Lezer dependency on the critical path.
 */

import { tags, Tag } from '@lezer/highlight';
import type { TokenThemeMapping } from '../../view-model/theme';
