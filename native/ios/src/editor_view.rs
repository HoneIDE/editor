//! EditorView: top-level state for an iOS editor instance.
//!
...
/// Null Objective-C pointer (replaces cocoa::base::nil on iOS).
...
/// Called when the user scrolls (pan gesture). `dx`/`dy` are pixel deltas.
...
impl EditorView {
    ...
    /// Get the underlying UIView handle.
    ...
    /// Called from the UIView's insertText: handler (UIKeyInput).
    ...
    /// Called from the UIView's deleteBackward handler or other action dispatches.
    ...
    /// Called from the UIView's touchesBegan: handler.
    ...
    /// Called from the UIView's touchesMoved: handler (pan gesture).
    ...
    ///
    /// # Safety
    /// Called from the UIView drawRect: handler with a valid CGContextRef.
...
impl Drop for EditorView {
    fn drop(&mut self) {
        if self.uiview != NIL {
            unsafe {
                let _: () = msg_send![self.uiview, removeFromSuperview];
            }
        }
    }
}
