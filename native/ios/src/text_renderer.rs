//! Core Text based text rendering for iOS.
//!
//! Shares the same CTFont / CTLine / CTLineDraw pipeline as macOS.
//! The only difference is the font fallback chain (Courier instead of Monaco,
//! since Monaco is not available on iOS).
...
impl FontSet {
    ...
    pub fn new(family: &str, size: f64) -> Self {
        let normal = ct_font::new_from_name(family, size)
            ...
            .or(ct_font::new_from_name("Courier", size))
...
/// Text matrix for drawing in a UIView context.
///
/// UIView uses a top-left origin coordinate system. The CGContext provided
/// by UIGraphicsGetCurrentContext() has a flipped y-axis compared to the
/// default Core Graphics coordinate system. Setting d = -1.0 flips the
/// glyphs in text space so they render correctly.
