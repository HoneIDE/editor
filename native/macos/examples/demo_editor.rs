//! Interactive demo: opens an NSWindow with a fully editable code editor.
//!
//! Run with: `cargo run --example demo_editor` from `native/macos/`
//!
...
use hone_editor_macos::{
    ...
    hone_editor_measure_text, hone_editor_nsview, hone_editor_render_line,
...
struct DemoEditor {
    ...
    /// When a line's text matches its original, the original tokens are used.
...
/// using common prefix/suffix alignment. Characters in the changed region
/// get default gray; unchanged characters keep their original colors.
fn validate_tokens_json(tokens_json: &str, orig_text: &str, curr_text: &str) -> String {
    ...
    // Build per-byte color/style arrays for current text
    ...
    // Parse each token and map to current positions
...
impl DemoEditor {
    ...
    /// Tokens whose text still matches the original keep their color;
    /// tokens that have been edited get default gray.
    ...
    fn click_to_cursor(&mut self, x: f64, y: f64) {
        let editor = self.editor_ptr as *mut hone_editor_macos::EditorView;
    ...
    fn copy_to_clipboard(&self) {
        ...
        unsafe {
            ...
            let _: () = msg_send![pb, setString: ns_str forType: string_type];
        }
    }
    ...
    fn paste_from_clipboard(&mut self) {
        let text = unsafe {
            let pb: id = msg_send![class!(NSPasteboard), generalPasteboard];
            let string_type = NSString::alloc(nil).init_str("public.utf8-plain-text");
            let ns_str: id = msg_send![pb, stringForType: string_type];
            if ns_str == nil {
                return;
            }
...
extern "C" fn on_text_input(
    _view: *mut hone_editor_macos::EditorView,
    text: *const c_char,
) {
...
extern "C" fn on_scroll(
    ...
    unsafe {
        if let Some(ref mut demo) = DEMO {
            // Trackpad/mouse: dy negative = scroll down (content moves up)
            demo.scroll_y -= dy;
            demo.clamp_scroll();
            demo.render();
        }
...
fn main() {
    unsafe {
        ...
        let title = NSString::alloc(nil).init_str("Hone Editor \u{2014} Interactive Demo");
        ...
        let nsview = hone_editor_nsview(editor);
        let _: () = msg_send![window, makeFirstResponder: nsview];
